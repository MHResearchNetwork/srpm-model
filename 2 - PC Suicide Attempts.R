...
setwd("C:/Users/Administrator/Desktop/pc")


sink("PC_sr_90_logfile.txt", split=TRUE)
...
srs2_analytic_subset <- read.csv("analytic_pc90.csv")
srs2_analytic_subset <- subset(srs2_analytic_subset, is.na(srs2_analytic_subset$event90)==0)
...
y <- srs2_analytic_subset$event90
wts <- rep(1, length(y))
...
write.csv(lambda.results, "PC_sr_90_Lambdas_m5.csv")
save(model.list, file="PC_sr_90_model_list_m5.RData")
save(final.model, file="PC_sr_90_final_model_m5.RData")
